...
const _rootStore = RootStoreModel.create({}, { ...extraArguments })
catchErrorMiddleware(_rootStore)
 ...
 */
export const useInitialRootStore = (callback: () => any | Promise<any>) => {
  const rootStore = useStores()
  ...
  useEffect(() => {
    let _unsubscribe
    ;(async () => {
      // set up the RootStore (returns the state restored from mmkv)
